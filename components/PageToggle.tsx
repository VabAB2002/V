...
export default function PageToggle({ className = '' }: PageToggleProps) {
    ...
    const isGenEd = pathname === '/gened';

    const handleToggle = (page: 'major' | 'gened') => {
        const targetPath = page === 'gened' ? '/gened' : '/';
    ...
    return (
        <div className={`inline-flex items-center bg-gray-100 rounded-full p-1 ${className}`}>
            <button
                ...
                className={`px-4 py-2 text-sm font-medium rounded-full transition-all duration-200 ${!isGenEd
