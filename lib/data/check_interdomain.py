...
# Interdomain courses from user's list
...
missing = []
present_no_interdomain = []
present_with_interdomain = []

for course in courses:
    cursor.execute("SELECT id, gen_ed_json FROM courses WHERE id = ?", (course,))
    row = cursor.fetchone()
    if row is None:
        missing.append(course)
    elif '"interdomain"' not in row[1]:
        present_no_interdomain.append((course, row[1]))
    else:
        present_with_interdomain.append(course)

print(f"=== INTERDOMAIN COURSES SUMMARY ===")
print(f"Total courses in list: {len(courses)}")
print(f"Present WITH interdomain: {len(present_with_interdomain)}")
print(f"Present but MISSING interdomain: {len(present_no_interdomain)}")
